...
const Card = styled.div`
  ...
  margin-bottom:12px;overflow:hidden;animation:${fadeUp} 0.2s ease;
`;
...
const ItemsSection = ({ items, newItem, onNIChange, onAdd, onClear, onEdit, onRemove, disabled }) => (
  <TScrollWrap>
    <ITable>
      <ITHead>
        <tr>
          <ITH style={{width:34}}>#</ITH><ITH>Item Name</ITH>
          <ITH style={{width:72}}>Qty</ITH><ITH style={{width:85}}>Rate</ITH>
          <ITH style={{width:80}}>Discount</ITH><ITH style={{width:92}}>Amount</ITH>
          <ITH>Doctor</ITH><ITH style={{width:72}}>Dr.Fee</ITH>
          <ITH>Description</ITH><ITH style={{width:78}}>Source</ITH><ITH style={{width:40}}></ITH>
        </tr>
      </ITHead>
      <tbody>
        <AddRow>
...
const DischargeBilling = () => {
  ...
  // null when creating fresh; { id, estimate_number } when editing an existing estimate
  const [editingEst, setEditingEst] = useState(null);

  const setForm  = (k,v) => setFormRaw(p=>({...p,[k]:v}));
  const totals   = calcTotals(items, form);

  // Refresh triggers for child list components
  ...
  const doSearch = async mode => {
    ...
    try {
      const param = mode==="uhid"?`uhid=${encodeURIComponent(val)}`:`ipNumber=${encodeURIComponent(val)}`;
  ...
  const handleNIChange = (field,value) => setNewItem(p=>recalcItem({...p,[field]:value},field,value));
  const handleAddItem  = () => {
  ...
  // ── "Edit / Convert" click in ViewEstimates ────────────────────────────────
  // Populates the Create tab with all estimate data, no modal.
  ...
  // ── Save as Final Bill (or Convert Estimate → Bill) ────────────────────────
  const handleSaveBill = async () => {
    ...
    try {
      const t = calcTotals(items,form);
      if (editingEst?.id) {
        // PATCH estimate first, then convert
        await apiRequest(`${BASE}discharge-billing/${editingEst.id}/`,"PATCH",buildPayload(items,form,t,"Estimate",patient));
        ...
        {tab==="create" && (<>
          ...
          <Card>
            ...
            <FinActions>
              ...
              {/* Amber button: "Update Estimate" when editing, "Save as Estimate" when new */}
              <Btn $amber onClick={handleSaveEstimate} disabled={saving||!patient}>
                {saving?<Spinner/>:"📋"} {editingEst?" Update Estimate":" Save as Estimate"}
              </Btn>

              {/* Primary/Purple button: "Convert & Save Bill" when editing, "Save as Final Bill" when new */}
              {editingEst ? (
