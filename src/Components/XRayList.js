  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content">
        <h4>X-Ray Report for {report.patientName}</h4>
        <p>
...
const XRayList = ({ patientId }) => {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/x_ray_investigations/")
  ...
  useEffect(() => {
    const fetchXRayReports = async () => {
      try {
        const url = patientId
          ? `http://localhost:8000/x_ray_reports/${patientId}/`
          : "http://localhost:8000/x_ray_reports/";

        const response = await fetch(url);

        if (!response.ok) {
          throw new Error("Error fetching XRay reports");
        }
        ...
        console.error("Error fetching XRay reports:", error);
      }
    };
  ...
  const handleDelete = async (report) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:8000/x_ray-reports/${encodedPatientId}/delete/`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleApprove = async (report) => {
    try {
      const encodedPatientId = encodeURIComponent(report.patientId);
      const response = await fetch(
        `http://localhost:8000/x_ray-reports/${encodedPatientId}/approve/`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="x_ray-list-container">
      <h1>X-Ray Investigations</h1>
      ...
      <h2>X-Ray Reports</h2>
      <table className="x_ray-report-table">
        <thead>
          <tr>
            <th>Patient ID</th>
            <th>Patient Name</th>
        ...
        <tbody>
          {Array.isArray(xrayReports) && xrayReports.length > 0 ? (
