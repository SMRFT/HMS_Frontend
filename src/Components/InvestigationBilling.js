...
const InvestigationBilling = () => {
  ...
  useEffect(() => {
    if (location.state?.patientData) {
      ...
      setFormData({
        EstBillNo: patientData.EstBillNo || "",
        EstBillDate: patientData.EstBillDate || "",
        ...
        billType: patientData.billType || "",
        item: patientData.item || [], // Ensure this is an array
        referredBy: patientData.referredBy || "",
        discountPercent: patientData.discountPercent || 0,
        discount: patientData.discount || "0.00",
        discountRemarks: patientData.discountRemarks || "",
        total: patientData.total || "0.00",
        finalPrice: patientData.finalPrice || "0.00",
        paymentMethod: patientData.paymentMethod || "cash",
      });

      setSelectedBillType(patientData.billType || ""); // Ensure Bill Type is set
      setProductList(patientData.item || []); // Set items to product list
    }
  }, [location.state]);
  ...
  const fetchIpPatient = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/ip-patient/${encodedipNumber}/`
      );
      if (response.ok) {
        const data = await response.json();
        setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("EstBillNo being sent for deletion:", formData.EstBillNo); // Debugging log

    // Extract EstBillNo for deletion but remove it from the new data
    const { EstBillNo, EstBillDate, ...filteredFormData } = formData;

    const formPayload = {
      ...filteredFormData,
      EstBillNo, // Include only for deletion
      item: productList.map((product) => ({
        itemName: product.itemName,
        price: product.price,
        quantity: product.quantity,
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Reset location state and reload
        navigate(location.pathname, { replace: true, state: {} });
        window.location.reload();
      } else {
        const errorData = await response.json();
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-3">
              ...
              >
                ...
                {[...new Set(billTypes.map((item) => item.billType))].map(
                  (billType, index) => (
          ...
          <ProductContainer>
            ...
            <ButtonGroup>
              ...
              <EstimateButton type="button" onClick={handleEstimate}>
                Make Estimate
              </EstimateButton>
