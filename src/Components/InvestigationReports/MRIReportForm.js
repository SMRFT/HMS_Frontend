...
const MRIReportForm = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    if (result.success) {
      toast.success('MRI report submitted successfully! ✓');
      navigate(-1);
    } else {
      toast.error(`Error submitting MRI report: ${result.error}`);
  ...
  if (!dataLoaded) {
    return (
      <PageContainer>
        <FormCard>
          <PageTitle>MRI Report Form</PageTitle>
          <ErrorMessage>
  ...
  return (
    <PageContainer>
      <FormCard>
        <PageTitle>MRI Report Form</PageTitle>
        <Subtitle>Complete the form below to submit a MRI investigation report</Subtitle>
