...
const MRIList = ({ patientId }) => {
  ...
  useEffect(() => {
    const fetchInvestigations = async () => {
      const result = await apiRequest(`${HMSURL}mri_investigations/`, "GET");
    ...
    const fetchMriReports = async () => {
      try {
        ...
        if (result.success) {
          ...
          setMriReports(reportsWithPatientName);
        } else {
          console.error("Error fetching MRI reports:", result.error);
          toast.error(result.error || "Failed to fetch MRI reports");
        }
      } catch (error) {
  ...
  const filterReportsByDate = useCallback(() => {
    ...
    const filtered = mriReports.filter(report => {
  ...
  }, [mriReports, fromDate, toDate]);

  // Apply date filter whenever MRI reports, fromDate, or toDate changes
  useEffect(() => {
    filterReportsByDate();
  }, [filterReportsByDate]);
  ...
  const handleDelete = async (report) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}mri-reports/delete/${encodedinvestBillNo}/`,
        "PATCH",
  ...
  const handleApprove = async (report) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}mri-reports/approve/${encodedinvestBillNo}/`,
        "PATCH",
        {
  ...
  const handleSaveEdit = async (newImpression) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}mri-reports/edit/${encodedinvestBillNo}/`,
        "PATCH",
        {
  ...
  return (
    <PageContainer>
      ...
      <ContentCard>
        <SectionHeader>
          <SectionIcon>📄</SectionIcon>
          <SectionTitle>MRI Reports</SectionTitle>
        </SectionHeader>
