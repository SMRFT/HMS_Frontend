...
const CTList = ({ patientId }) => {
  ...
  useEffect(() => {
    const fetchInvestigations = async () => {
      const result = await apiRequest(`${HMSURL}investigations/`, "GET");
    ...
    const fetchCtReports = async () => {
      try {
        ...
        if (result.success) {
          ...
          setCtReports(reportsWithPatientName);
        } else {
          console.error("Error fetching CT reports:", result.error);
          toast.error(result.error || "Failed to fetch CT reports");
        }
      } catch (error) {
  ...
  const filterReportsByDate = useCallback(() => {
    ...
    const filtered = ctReports.filter(report => {
  ...
  }, [ctReports, fromDate, toDate]);

  // Apply date filter whenever CT reports, fromDate, or toDate changes
  useEffect(() => {
    filterReportsByDate();
  }, [filterReportsByDate]);
  ...
  const handleDelete = async (report) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}ct-reports/delete/${encodedinvestBillNo}/`,
        "PATCH",
  ...
  const handleApprove = async (report) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}ct-reports/approve/${encodedinvestBillNo}/`,
        "PATCH",
        {
  ...
  const handleSaveEdit = async (newImpression) => {
    try {
      ...
      const result = await apiRequest(
        `${HMSURL}ct-reports/edit/${encodedinvestBillNo}/`,
        "PATCH",
        {
  ...
  return (
    <PageContainer>
      ...
      <ContentCard>
        <SectionHeader>
          <SectionIcon>📄</SectionIcon>
          <SectionTitle>CT Reports</SectionTitle>
        </SectionHeader>
