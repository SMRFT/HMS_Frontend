...
const IPGRNGeneration = () => {
  ...
  const handleItemChange = (e) => {
    ...
    // If medicine is selected from stock, auto-fill some fields
    if (name === "name" && value !== "") {
  ...
  useEffect(() => {
    ...
    setGrnData(prev => ({
      ...
      non_taxable_amount: summary.taxable_amount.toFixed(2), // Following user example's weird labelling
  ...
  const saveGRN = async () => {
    ...
    try {
      const response = await apiRequest(`${baseUrl}ip-grn/`, "POST", payload)
      if (response.success) {
  ...
  return (
    <PageWrapper>
      <Container>
        <FormContent>
          <SectionTitle><h3>IP Pharmacy GRN Generation</h3></SectionTitle>
          ...
          <div style={{ backgroundColor: 'white', padding: '24px', borderRadius: '8px', marginBottom: '24px', boxShadow: '0 4px 6px rgba(0, 0, 0, 0.05)' }}>
            ...
            <FormRow style={{ alignItems: 'flex-end', backgroundColor: '#fdfdfd', padding: '15px', borderRadius: '8px', border: '1px dashed #cbd5e1' }}>
              <InputWrapper style={{ minWidth: '200px' }}>
                ...
                <Input list="ip-medicines" name="name" value={currentItem.name} onChange={handleItemChange} placeholder="Select or Type Name" />
                <datalist id="ip-medicines">
                  {stocks.map((s, idx) => (
...
export default IPGRNGeneration
