...
const SummaryStrip = styled.div`
  ...
  gap: 8px; background: #f8fafc; border: 1px solid ${colors.border};
...
const GRNGeneration = () => {
  ...
  // round_amount change → recalc net
  useEffect(()=>{
  ...
  /* ── Vendor change — auto-fill address / contact / phone ── */
  ...
  const saveGRN = async () => {
    ...
    try {
      const r = await apiRequest(
        editId ? `${baseUrl}grn/${editId}/` : `${baseUrl}grn/`,
        editId ? "PUT" : "POST",
        payload
      )
      if(r.success){
        const draftNo = r.data?.draft_number || ""
        toast.success(editId ? `GRN updated (${draftNo})` : `Draft saved: ${draftNo}`)
        resetForm(); fetchGRNList(); setActiveTab("list")
      } else {
  ...
  const resetForm = () => {
    ...
    setMedSearch(""); setEditId(null); setEditDraftNo(""); setVendorInfo(null)
  }

  /* ── Helpers ── */
  const vendorAddr    = vendorInfo
          ...
          {activeTab==="create" && (
            <>
              ...
              <Card>
                ...
                <CardBody>
                  {/* Row 1: Category | Date | Vendor */}
                  <GridRow cols="repeat(3,1fr)">
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Purchase Category *</Lbl>
                      <Select name="purchase_category" value={grnData.purchase_category} onChange={handleGrnChange} style={{fontSize:"0.8rem"}}>
                    ...
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Date</Lbl>
                      <Input type="date" name="date" value={grnData.date} onChange={handleGrnChange} style={{fontSize:"0.8rem"}} />
                    </InputWrapper>
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Vendor *</Lbl>
                      <Select name="vendor_id" value={grnData.vendor_id} onChange={handleVendorChange} style={{fontSize:"0.8rem"}}>
                  ...
                  {/* Row 2: Supplier Address | Contact | Phone */}
                  <GridRow cols="repeat(3,1fr)">
                    <InputWrapper style={{margin:0}}>
                      ...
                      <ReadOnlyInput
                        value={vendorInfo
                          ? [vendorInfo.address_line1,vendorInfo.address_line2,vendorInfo.city,vendorInfo.state].filter(Boolean).join(", ")
                          : ""}
                        readOnly placeholder="Auto-filled from vendor"
                  ...
                  {/* Row 3: Type | Invoice No | Invoice Date | Payment Mode */}
                  <GridRow cols="repeat(4,1fr)" style={{marginBottom:0}}>
                    ...
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Invoice No *</Lbl>
                      <Input name="invoice_no" value={grnData.invoice_no} onChange={handleGrnChange} placeholder="e.g. INV-52412" style={{fontSize:"0.8rem"}} />
                    </InputWrapper>
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Invoice Date</Lbl>
                      <Input type="date" name="invoice_date" value={grnData.invoice_date} onChange={handleGrnChange} style={{fontSize:"0.8rem"}} />
                    </InputWrapper>
                    <InputWrapper style={{margin:0}}>
                      <Lbl>Payment Mode</Lbl>
                      <Select name="payment_mode" value={grnData.payment_mode} onChange={handleGrnChange} style={{fontSize:"0.8rem"}}>
              ...
              <Card>
                ...
                <CardBody>
                  ...
                  {items.length>0&&(
                    <ScrollTable style={{marginTop:12}}>
              ...
              <ButtonContainer>
                ...
                <Button onClick={saveGRN} disabled={loading}>
                  <ShoppingCart size={13}/>&nbsp;{loading?"Saving…":editId?"Update Draft":"Save as Draft"}
                </Button>
              </ButtonContainer>
            </>
          )}
