...
const IPPharmacyStock = () => {
    ...
    const fetchStocks = async () => {
        try {
            const response = await apiRequest(`${baseUrl}ip-pharmacy-stock/`, "GET")
            if (response.success) {
    ...
    const handleInputChange = (e) => {
        ...
        if (["purchase_rate", "total_quantity", "cgst_rate", "sgst_rate"].includes(name)) {
            ...
            // Taxable amount is often same as cost or cost before tax
            // User's example: purchase_rate: 12, cost: 1200, qty: 200 (wait 12*200=2400 but example says 1200)
            // Ah, example: rate 12, cost 1200, qty 200. Maybe rate is per pack or something.
            // Let's just follow price * qty = cost logic unless taxable_amount is manually entered.
            // Actually, let's keep it simple as per user's example values if they are related.

            const cgstRate = parseFloat(updatedData.cgst_rate) || 0
            const sgstRate = parseFloat(updatedData.sgst_rate) || 0

            // User example: taxable_amount 1190, cgst_rate 6, cgst_amount 71.4 (1190 * 0.06 = 71.4) Correct!
    ...
    const handleSave = async () => {
        try {
            ...
            if (isEditing && editingId) {
                response = await apiRequest(`${baseUrl}ip-pharmacy-stock/${editingId}/`, "PATCH", formData)
            } else {
                response = await apiRequest(`${baseUrl}ip-pharmacy-stock/`, "POST", formData)
            }
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const response = await apiRequest(`${baseUrl}ip-pharmacy-stock/${id}/`, "DELETE")
            if (response.success) {
    ...
    const handleEdit = (stock) => {
        setFormData({ ...stock })
        setIsEditing(true)
        setEditingId(stock.id || stock.ip_stock_id || stock._id?.$oid)
    ...
    const filteredStocks = stocks.filter(
        (stock) =>
            ...
            stock.ip_stock_id?.includes(search),
    )

    return (
        <PageWrapper>
            <Container>
                <SectionTitle><h3>IP Pharmacy Stock Management</h3></SectionTitle>
                ...
                <SectionTitle><h3>IP Stock List</h3></SectionTitle>
                        ...
                        <tbody>
                            {filteredStocks.length === 0 ? (
                                ...
                                filteredStocks.map((stock) => (
                                    <Tr key={stock.id || stock.ip_stock_id || stock._id?.$oid}>
                                        <Td>{stock.ip_stock_id}</Td>
                                        ...
                                        <Td>
                                            <div style={{ display: 'flex', gap: '8px' }}>
                                                ...
                                                <FaTrash
                                                    ...
                                                    onClick={() => handleDelete(stock.id || stock.ip_stock_id || stock._id?.$oid)}
                                                    title="Delete"
                                                />
                                            </div>
                                        </Td>
...
export default IPPharmacyStock
