...
const Badge = styled.span`
  ...
  font-size: 0.75rem;
  font-weight: 600;
  background: ${(p) => (p.type === "SUPPLIER" ? "#dbeafe" : p.type === "BOTH" ? "#e0e7ff" : "#fef3c7")};
  color: ${(p) => (p.type === "SUPPLIER" ? "#1d4ed8" : p.type === "BOTH" ? "#4338ca" : "#92400e")};
`;
...
const SUPPLIER_TYPES = ["SUPPLIER", "MANUFACTURER", "BOTH"];
const PAYMENT_TERMS = ["CHEQUE", "CASH", "DD", "NET BANKING", "CREDIT 30 DAYS"];
const COUNTRY = "India";
...
const Generalstorevendor = () => {
  ...
  const fetchVendors = useCallback(async () => {
    try {
      const response = await apiRequest(`${baseUrl}general-store-vendors/`, "GET");
      if (response.success) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const url = editId
        ? `${baseUrl}general-store-vendors/${editId}/`
        : `${baseUrl}general-store-vendors/`;
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this store vendor?")) return;
    try {
      const response = await apiRequest(`${baseUrl}general-store-vendors/${id}/`, "DELETE");
      if (response.success) {
  ...
  return (
    <PageWrapper>
      ...
      <Container>
        ...
        <PageHeader>
          <div>
            <PageTitle>🏪 General Store Vendor Management</PageTitle>
            <PageSubtitle>Manage general store suppliers and manufacturers</PageSubtitle>
          </div>
          {!showForm && (
            <Button
        ...
        <FormContent>
          ...
          {showForm && (
            <FormCard>
              <FormCardHeader>
                🏢 {editId ? "Edit General Store Vendor" : "Add New General Store Vendor"}
              </FormCardHeader>
              <div style={{ padding: "20px" }}>
                {/* Row 1 */}
                <FormRow>
                  <InputWrapper>
                    <Label required>Supplier / Manufacturer / Both</Label>
                    <Select
                      name="vendor_type"
          ...
          {/* ── Table ── */}
          <SectionTitle>
            <h3>General Store Vendor List</h3>
          </SectionTitle>
          ...
          <ControlsContainer>
            <SearchContainer>
              <InputWrapper>
                <Label>Search</Label>
                <SearchInput
                  placeholder="Search by ID, name, type, GSTIN, city..."
                  value={search}
              ...
              <tbody>
                {pageData.length === 0 ? (
                  <tr>
                    <td colSpan={11}>
                      <EmptyState>
                        No store vendors found. Click "Add Store Vendor" to get started.
                      </EmptyState>
                    </td>
                  </tr>
                ) : (
                  pageData.map((vendor, idx) => (
                    <Tr key={vendor.vendor_id}>
                      ...
                      <Td style={{ fontFamily: "monospace", fontWeight: 600 }}>{vendor.vendor_id}</Td>
                      <Td style={{ fontWeight: 600, minWidth: 160 }}>{vendor.name}</Td>
                      <Td>
                        <Badge type={vendor.vendor_type}>
                          {vendor.vendor_type}
                        </Badge>
                      </Td>
                      <Td style={{ maxWidth: 180, fontSize: "0.82rem" }}>
                        {[vendor.address_line1 || vendor.address_line_1, vendor.address_line2 || vendor.address_line_2]
                          .filter(Boolean)
                          .join(", ")}
                      </Td>
                      <Td>
...
export default Generalstorevendor;
