...
const AssetsMaintainance = () => {
    ...
    const fetchAssets = useCallback(async () => {
        ...
        const res = await apiRequest(`${baseurl}stores-assets-maintainance/${query}`);
        if (res.success) {
            setAssets(res.data);
        }
        setLoading(false);
    ...
    const handleUniversalSearch = async () => {
        ...
        setLoading(true);
        const res = await apiRequest(`${baseurl}stores-assets-maintainance/${filters.search_barcode}/`);
        if (res.success) {
            setAssets([res.data]);
        } else {
    ...
    const handleEditForm = async (pk) => {
        ...
        const res = await apiRequest(`${baseurl}stores-assets-maintainance/${pk}/`);
    ...
    const handleSave = async () => {
        ...
        const url = isEditing 
            ? `${baseurl}stores-assets-maintainance/${currentId}/` 
            : `${baseurl}stores-assets-maintainance/`;
    ...
    const activateAsset = async (id) => {
        const res = await apiRequest(`${baseurl}stores-assets-maintainance/${id}/`, 'PATCH', { is_active: true, deactivated_date: null, deactivate_remarks: "" });
        if (res.success) {
    ...
    const confirmDeactivate = async () => {
        ...
        const res = await apiRequest(`${baseurl}stores-assets-maintainance/${assetToDeactivate.asset_id}/`, 'PATCH', { is_active: false, deactivate_remarks: deactivateReason });
                            ...
                            <>
                                ...
                                <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '15px'}}>
                                    ...
                                    <Button style={{ fontSize: '0.75rem', padding: '4px 10px' }} onClick={() => {
                                        const updated = [...formData.maintainance_details, { service_date: today, service_cost: 0, service_description: '' }];
                                        setFormData({...formData, maintainance_details: updated});
                                    }}>
                                        <Plus size={14}/> Add Service
                                    </Button>
                                </div>

                                {formData.maintainance_details && formData.maintainance_details.map((service, idx) => (
                                    <FormRow key={idx} style={{background: '#ffffff', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', marginBottom: '10px'}}>
                                        <InputWrapper>
                                            ...
                                            <Input type="date" value={service.service_date} onChange={e => {
                                                const updated = [...formData.maintainance_details];
                                        ...
                                        <Button onClick={() => {
                                             const updated = formData.maintainance_details.filter((_, i) => i !== idx);
                                             setFormData({...formData, maintainance_details: updated});
                    ...
                    <TableWrapper style={{ marginTop: '15px' }}>
                        {loading ? <div style={{textAlign:'center', padding:'20px'}}><Loader className="animate-spin"/></div> : (
                            <Table>
                                ...
                                <tbody>
                                    {assets.map(asset => {
                                        const logs = asset.maintainance_details || [];
                ...
                <S.ModalContainer style={{ maxWidth: '600px' }}>
                    ...
                    <ModalBody>
                        ...
                        <div>
                            ...
                            {(selectedViewAsset.maintainance_details && selectedViewAsset.maintainance_details.length > 0) ? (
                                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {selectedViewAsset.maintainance_details.map((log, i) => (
