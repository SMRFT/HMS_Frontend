...
const USGReportForm = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/usg_investigations/${uhid}/${subUhid}/`)
  ...
  const handleSubmit = (event) => {
    ...
    fetch("http://localhost:8000/usg-reports/", {
      method: "POST",
      headers: {
      ...
      .then((response) => response.json())
      .then((data) => {
        alert("USG report submitted successfully");
      ...
      .catch((error) => {
        alert("Error submitting USG report");
