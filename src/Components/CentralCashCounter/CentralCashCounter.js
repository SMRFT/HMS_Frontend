...
export default function CentralCashCounter() {
  ...
  const [pvToDate, setPvToDate] = useState(
  ...
  const rpFetchAccountHeads = async () => {
    try {
      const res = await apiRequest(
        `${HmsBaseUrl}get_active_account_heads/`,
        "GET",
      );
  ...
  const shiftBelongsHere = !!(
    activeShift &&
    activeShift.is_active === true &&
  ...
  const refreshActiveShift = async () => {
    try {
      ...
      const res = await apiRequest(`${HmsBaseUrl}get_active_shift/`, "POST", {
                      ...
                      {rpAlert && (
                        <div
                          style={{
                            ...
                            fontSize: 14,
                            backgroundColor:
                              rpAlert.type === "error" ? "#fef2f2" : "#f0fdf4",
                            color:
                              rpAlert.type === "error" ? "#dc2626" : "#166534",
                            ...
                            >
                              ...
                              <Select
                                ...
                                onChange={(e) => {
                                  ...
                                  const head = Array.isArray(rpAccountHeads)
                                    ? rpAccountHeads.find(
                                        (h) => h["S.No"] === e.target.value,
                                      )
                                    : null;
                                ...
                                style={{ minWidth: 200 }}
                              >
                                {(!Array.isArray(rpAccountHeads) ||
                                  rpAccountHeads.length === 0) && (
                                  <option value="">Loading...</option>
                                )}
                                {Array.isArray(rpAccountHeads) &&
                                  rpAccountHeads.map((h) => (
                              ...
                              >
                                ...
                                >
                                  ...
                                  <select
                                    value={pvShowEntries}
                                    onChange={(e) => {
                                      setPvShowEntries(Number(e.target.value));
                                      setPvPage(1);
                                    }}
                                    style={{
                                      border: "1px solid #d1d5db",
        ...
        (() => {
          ...
          const descText = v.description
            ? typeof v.description === "object"
              ...
              : v.description
            : "—";
          const amount = parseFloat(v.amount || 0).toLocaleString("en-IN", {
            minimumFractionDigits: 2,
          });
          const hospitalName =
          ...
          return (
            <ModalOverlay onClick={() => setRpPrintVoucher(null)}>
              ...
              >
                {/* Header */}
                <ModalHeader
                  style={{ background: "#0d9488", borderRadius: "8px 8px 0 0" }}
                >
                  <ModalTitle style={{ fontSize: 16 }}>
                    🖨️ Central Cash Voucher
                  </ModalTitle>
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <CloseButton onClick={() => setRpPrintVoucher(null)}>
                      ✕
                    </CloseButton>
                  </div>
                </ModalHeader>
                ...
                <ModalFooterBar>
                  <CancelButton onClick={() => setRpPrintVoucher(null)}>
                    Close
                  </CancelButton>
                  <SaveButton
                    onClick={() => {
